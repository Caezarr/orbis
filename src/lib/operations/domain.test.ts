...
const row: TaskRow = {
  ...
  workflow_id: "rental-operations",
  task_id: "read-only:request-analysis",
  title: "Guest reply",
  status: "needs_review",
  created_at: new Date("2026-09-14T10:00:00Z"),
  completed_at: null,
  ready_at: new Date("2026-09-14T10:01:00Z"),
  output: {
    title: "Reply",
    body: "Draft",
    citations: [],
  ...
  quote: {
    workflowId: "rental-operations",
    taskId: "read-only:request-analysis",
    quantity: 1,
    currency: "EUR",
    unitPriceCents: 150,
    totalCents: 150,
...
describe("operational task contract", () => {
  it("requires explicit quote acceptance", () => {
    expect(
      enqueueSchema.safeParse({
        missionId: "mission",
        workflowId: "rental-operations",
        text: "Prepare a guest response",
      }).success,
    ).toBe(false);
    expect(
      enqueueSchema.safeParse({
  ...
  it("binds review token to exact result and price", () => {
    ...
    expect(
      publicTask({ ...row, quote: { ...row.quote, totalCents: 200 } })
        .reviewToken,
    ).not.toBe(before);
    expect(
      publicTask({ ...row, output: { ...row.output!, body: "Changed" } })
        .reviewToken,
    ).not.toBe(before);
