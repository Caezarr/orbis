...
describe("durable read-only worker", () => {
  ...
  it("verifies membership before reading a task", async () => {
    ...
    await expect(runOneTask(identity, vi.fn())).rejects.toThrow(
      "Worker membership required",
    );
  });
  it("does nothing when another worker holds the available tasks", async () => {
    mock.query.mockImplementation(async (sql: string) => ({
      rowCount: 1,
      rows: sql.startsWith("SELECT 1") ? [{}] : [],
    }));
    const generate = vi.fn();
    expect(await runOneTask(identity, generate)).toEqual({ processed: false });
    expect(generate).not.toHaveBeenCalled();
  });
  it("marks failed preparation non-billable without accepting output", async () => {
    await runOneTask(
      identity,
      vi.fn().mockRejectedValue(new Error("provider key must never leak")),
    );
    const update = mock.query.mock.calls.find(([sql]) =>
      sql.includes("output=$4"),
    );
    expect(update?.[1][2]).toBe("failed");
    expect(JSON.stringify(update)).not.toContain("provider key");
  ...
  it("leaves a generated result for review, never auto-accepts", async () => {
    ...
    expect(
      mock.query.mock.calls.some(([sql]) =>
        sql.includes("INSERT INTO operational_acceptances"),
      ),
    ).toBe(false);
  });
});
