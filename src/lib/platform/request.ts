...
export function blankTenantState(workspace: Workspace): StoreState {
  return {
    ...
    impact: {
      hoursSaved: 0,
      acceptedResultCostEur: 0,
      secondCapabilityMinutes: 0,
      correctionRate: 0,
      runsThisWeek: 0,
    },
  };
...
async function resolveContext(client: PoolClient): Promise<WorkspaceContext> {
  ...
  let rows = (
    await client.query<MemberRow>(
      "SELECT * FROM memberships WHERE user_id = $1 ORDER BY created_at, workspace_id",
      [user.id],
    )
  ).rows;
  if (!rows.length) {
    ...
    await client.query(
      ...
      [id, tenantId, workspace.name, workspace.slug, user.id],
    );
    rows = (
      await client.query<MemberRow>(
        "INSERT INTO memberships(id,workspace_id,tenant_id,user_id,name,email,role) VALUES($1,$2,$3,$4,$5,$6,'owner') RETURNING *",
        [
          randomUUID(),
          id,
          tenantId,
  ...
  const membership = selected
    ? rows.find((row) => row.workspace_id === selected)
    : rows[0];
  if (!membership)
    throw new PlatformError("Workspace membership required", 403);
  const identity = {
    userId: user.id,
  ...
  const result = await client.query<{ state: StoreState }>(
    "SELECT state FROM workspace_state WHERE workspace_id=$1 AND tenant_id=$2 FOR UPDATE",
    [identity.workspaceId, identity.tenantId],
  );
  if (!result.rows[0])
    throw new PlatformError("Workspace state unavailable", 503);
  const state = result.rows[0].state;
  if (
    state.workspace.id !== identity.workspaceId ||
    state.workspace.tenantId !== identity.tenantId
  )
    throw new PlatformError("Workspace state identity mismatch", 503);
  // Authorization always uses relational membership, never editable snapshot data.
  state.memberships = rows
    .filter((row) => row.workspace_id === identity.workspaceId)
    .map(member);
  return {
    ...identity,
    role: membership.role,
...
export type WorkspaceRequestOptions = {
  requireRole?: Membership["role"] | Membership["role"][];
};
function enforceRole(
  ...
  const roles = Array.isArray(options.requireRole)
    ? options.requireRole
    : [options.requireRole];
  if (context.role !== "owner" && !roles.includes(context.role))
    throw new PlatformError("Insufficient workspace role", 403);
}
...
export async function withWorkspaceRequest(
  request: Request,
  ...
  try {
    ...
    if (mutation && !isSameOriginMutation(request))
      throw new PlatformError("Same-origin request required", 403);
    if (isOfflineMode()) {
      const url = new URL(request.url);
      if (!["localhost", "127.0.0.1", "[::1]"].includes(url.hostname))
        throw new PlatformError("Offline mode is localhost-only", 403);
    ...
    return await transaction(async (client) => {
      ...
      try {
        return await workspaceStorage.run(context, async () => {
          ...
          if (context.dirty) {
            if (
              context.state.workspace.id !== context.workspaceId ||
              context.state.workspace.tenantId !== context.tenantId
            )
              throw new PlatformError("Workspace identity cannot change", 409);
            await client.query(
              "UPDATE workspace_state SET state=$1::jsonb, version=version+1, updated_at=now() WHERE workspace_id=$2 AND tenant_id=$3",
              [
          ...
          const events: Record<string, EventName> = {
            "/api/v1/company-analysis": "company_analyzed",
            "/api/v1/orbi": "orbi_answered",
            "/api/v1/sources": "knowledge_source_added",
            "/api/v1/knowledge-scopes": "knowledge_selected",
          };
          const event =
            events[path] ??
            (/^\/api\/v1\/memory\/[^/]+$/.test(path)
              ? "memory_reviewed"
              : /^\/api\/v1\/connections\/[^/]+\/start$/.test(path)
                ? "connection_started"
                : /^\/api\/v1\/business-audits\/[^/]+\/install$/.test(path)
                  ? "workflow_installed"
                  : undefined);
          if (mutation && event)
            await track(event, {
...
export async function getRequestStore(): Promise<StoreState> {
  ...
  try {
    return await transaction(async (client) =>
      structuredClone((await resolveContext(client)).state),
    );
  } catch (error) {
