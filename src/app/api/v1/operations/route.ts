...
export async function GET(request: Request) {
  return withWorkspaceRequest(request, async () => {
    ...
    if (url.searchParams.has("missionId"))
      return Response.json({
        quote: quoteFor(
...
export async function POST(request: Request) {
  return withWorkspaceRequest(
    request,
    async () => {
      if (process.env.ORBIS_OPERATIONS_ENABLED !== "true")
        return Response.json(
          { error: "Task execution is not enabled for this deployment." },
          { status: 503 },
        );
      ...
      if (!parsed.success)
        return Response.json(
          { error: "Review the mission, input and quoted task price." },
          { status: 400 },
        );
      return Response.json(
        await enqueue(
