...
const idOf = (value: string | { id: string }) =>
  ...
  try {
    await transaction(async (db) => {
      ...
      if (
        ...
        if (delivered.metadata?.task_batch_id) {
          const batch = (
            await db.query(
              "SELECT * FROM task_payment_batches WHERE id=$1 FOR UPDATE",
              [delivered.metadata.task_batch_id],
            )
          ).rows[0];
          if (!batch) throw new Error("Task payment reservation unavailable");
          const session = await stripe().checkout.sessions.retrieve(
            delivered.id,
          );
          if (
            session.client_reference_id !== batch.id ||
            !session.customer ||
            idOf(session.customer) !== batch.customer_id ||
            session.amount_total !== batch.amount_cents ||
            session.currency !== "eur" ||
            session.mode !== "payment" ||
            (batch.session_id && batch.session_id !== session.id)
          )
            throw new Error("Task payment does not match reservation");
          const status =
            session.payment_status === "paid"
              ...
              : session.status === "expired"
                ...
                : session.status === "complete"
                  ...
                  : "open";
          await db.query(
            "UPDATE task_payment_batches SET session_id=$2,status=$3,paid_at=CASE WHEN $3='paid' THEN COALESCE(paid_at,now()) ELSE paid_at END WHERE id=$1 AND status<>'paid'",
            [batch.id, session.id, status],
          );
          return;
        }
      }
      if (
        ...
        await db.query(
          "UPDATE stripe_checkout_attempts SET expires_at=now() WHERE session_id=$1",
          [session.id],
        );
      }
      if (
      ...
      const owner = await db.query<{ tenant_id: string }>(
      ...
      if (
        ...
        throw new Error("Multiple active subscriptions require reconciliation");
      await db.query(
        `INSERT INTO stripe_subscriptions(tenant_id,stripe_subscription_id,stripe_customer_id,status,plan,seats,current_period_end,cancel_at_period_end)
    ...
    return Response.json(
      { error: "Billing synchronization unavailable. Retry this event." },
      { status: 503 },
    );
  }
}
